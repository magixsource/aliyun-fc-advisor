...
/**
 * Tag Food Query Controller
 *
 * @author lin.peng
 * @since 1.0
 **/
public class TagFoodQueryController extends FunctionController<BaseQueryRequest, ServerlessRequest, ServerlessResponse> implements PojoRequestHandler<BaseQueryRequest, ServerlessResponse> {
    private static final Logger logger = LoggerFactory.getLogger(TagFoodQueryController.class);
    ...
    public ServerlessResponse internalHandle(BaseQueryRequest dto) {
        ...
        String tagName = dto.getQ();
        if (tagName == null || tagName.isEmpty()) {
            throw new IllegalArgumentException("Bad request.");
        }
        PageInfo pageInfo = healthQueryApi.queryFoodByTagName(tagName, dto.getPageSize(), dto.getPage());
