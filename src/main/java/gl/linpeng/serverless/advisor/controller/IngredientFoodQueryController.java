...
/**
 * Ingredient food query controller
 *
 * @author lin.peng
 * @since 1.0
 **/
public class IngredientFoodQueryController extends FunctionController<BaseQueryRequest, ServerlessRequest, ServerlessResponse> implements PojoRequestHandler<BaseQueryRequest, ServerlessResponse> {
    private static final Logger logger = LoggerFactory.getLogger(IngredientFoodQueryController.class);
    ...
    public ServerlessResponse internalHandle(BaseQueryRequest dto) {
        ...
        Long ingredientId = dto.getId();
        if(ingredientId == null){
            throw new IllegalArgumentException("Bad request.");
        }

        PageInfo pageInfo = healthQueryApi.queryFoodByIngredientId(ingredientId, dto.getPageSize(), dto.getPage());
